...
vector<Rect> divideHW( Mat src, int dim, double threshold1, double threshold2 )
{
    ...
    vector<Rect> rects;         // vector/array of rectangles

    Rect rect(0,0,gray.cols,gray.rows);  // single rectangle initialized with these vals
    if( !pts.size() )
    {
        rects.push_back( rect );        // append the rectangle we just initialized into the array of rectangles 
    }
    int ref_x = 0;
    int ref_y = 0;
